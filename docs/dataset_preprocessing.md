...
7. **图打包**
   ...
   - 预处理脚本会同时生成 `summary.json` 统计各划分图数量、节点规模、攻击比率，并新增 `synthetic_graphs` 计数与 `diagnostics` 详情，用于标记由于窗口不足而复制的样本来源。
...
4. **标签与划分**
  ...
  - 当窗口数量极少导致验证/测试集为空时，预处理脚本会自动从现有窗口复制样本填充并在
    `summary.json` 的 `synthetic_graphs` 与 `diagnostics` 字段标注来源；训练脚本在运行时仍会
    再次检查，如仍存在空划分则会打印警告并重复上述复制逻辑，仅用于诊断，建议通过减
    小 `--window-size` 或 `--stride` 重新预处理，以获得可评估的划分。
  - 确保每类攻击在测试集中出现。
