...
4. **标签与划分**
  ...
  - 当窗口数量极少导致验证/测试集为空时，预处理脚本会在 `diagnostics.empty_splits` 中给出提示；训练脚本会打印警告并临时复制训练集图构造验证/测试划分（指标 JSON 中 `synthetic_split=true`），以便流程继续运行。请尽快重新运行预处理，减小 `--window-size`、`--stride` 或放宽 `--min-nodes` 生成更多窗口，以获得可评估的划分。
  - 确保每类攻击在测试集中出现。
