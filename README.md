...
> ```
> 若某一划分（例如 TON_IoT 的验证/测试集）因窗口数量过少而为空，训练脚本会先尝试
> 从已有窗口自动重采样生成验证/测试集合，并给出提示信息，确保日志中仍然包含对
> 应指标。若总窗口数不足 3 个，则会保留 `n/a (empty split)` 提示，此时可减小
> `--window-size` 或 `--stride` 以生成更多窗口，再次运行预处理即可获得完整评估。
